...
def test_promotion_requires_painful_review_and_measurable_gain():
    base = {"baseline": {"failure_rate": 0.18}, "changed": {"failure_rate": 0.09}, "directions": {"failure_rate": "lower"}, "change": "retry gate", "test": {"passed": True}, "evaluation": {"measurable_gain": True}, "comparison": {"status": "IMPROVED"}}
...
def test_regression_always_requires_rollback():
    assert assess_promotion({
        "baseline": {"quality": 0.8}, "changed": {"quality": 0.7}, "change": "new route", "test": {"passed": True},
        "evaluation": {"regression": True}, "comparison": {"status": "REGRESSION"},
...
def test_string_false_cannot_grant_promotion():
    result = assess_promotion({
        "baseline": {"quality": 0.8}, "changed": {"quality": 0.8}, "change": "new route", "test": {"passed": "false"},
        "evaluation": {"measurable_gain": "false"}, "comparison": {"status": "NO_MEASURABLE_GAIN"},
...
def test_video_receipt_ingestion_is_idempotent(tmp_path):
    ...
    run.write_text(json.dumps({
        ...
        "records": [{"source_id": "pyscenedetect", "status": "NEW_OR_CHANGED", "source_content_key": "pyscenedetect:abc", "readme_sha256": "abc", "readme_url": "https://example/readme"}],
    }), encoding="utf-8")
...
def test_same_source_content_key_is_deduplicated_across_runs(tmp_path):
    packet = make_packet(scope="video", candidate={"candidate_id": "run-1", "title": "x", "source_kind": "video_receipt", "source_status": "NEW_OR_CHANGED", "source_content_key": "x:v1"})
    packet2 = make_packet(scope="video", candidate={"candidate_id": "run-2", "title": "x", "source_kind": "video_receipt", "source_status": "NEW_OR_CHANGED", "source_content_key": "x:v1"})
    out = tmp_path / "bridge.jsonl"
...
def test_packet_hash_is_verified_before_runtime_consumption():
    packet = make_packet(scope="video", candidate={"candidate_id": "x", "title": "x", "source_kind": "video_receipt", "source_content_key": "x:v1", "source_status": "NEW_OR_CHANGED"})
    assert validate_packet(packet)["packet_id"] == packet["packet_id"]
    packet["title"] = "tampered"
    import pytest
    with pytest.raises(ValueError, match="hash|id"):
