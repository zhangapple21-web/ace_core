...
是**一个统一身份、多个生态位**的认知生态：以可重建的结构与意图承载连续性，
而不是依附于某个常驻进程、某个插件或某个模型住户。

观察者、研究者、验证者、档案官是可恢复的责任位置；模型、Provider 与工具只在
具体问题出现时作为可替换执行资源参与。表现可以变化，但身份连续性不属于任何
单一模型或运行时。
...
6. **生态母盘，不是常驻住户** — 保留可重建的边界、记忆、观察、路由、恢复与
   谱系；角色不绑定模型，领域不反向定义内核
